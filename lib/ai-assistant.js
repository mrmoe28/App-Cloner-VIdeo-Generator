...
class AIVideoAssistant {
  ...
  async getOpenAIClient(userId = 'default') {
    ...
    try {
      ...
      logger.warn('Failed to decrypt API key, assuming plain text', { 
        error: error.message,
        context: 'API key initialization'
      });
      decryptedKey = apiKey;
    }
  ...
  async generateVideoScript({ prompt, platform = 'general', duration = 60, userId = 'default' }) {
    try {
      ...
      try {
        ...
        logger.warn('Failed to parse AI script as JSON, using fallback', {
  ...
  async improveScript({ script, improvements = ['engagement', 'clarity'], userId = 'default' }) {
    try {
      ...
      try {
        ...
        logger.warn('Failed to parse improved script as JSON', {
          error: parseError.message,
          originalScriptTitle: script.title
        });
        return {
          ...script,
