...
class VideoGeneratorService {
  ...
  async generateVideo({ script, options = {} }) {
    ...
    try {
      ...
      const visualAssets = await this.generateVisualAssets(sceneAnalysis, projectDir, options.fastMode);
  ...
  async generateVisualAssets(sceneAnalysis, projectDir, fastMode = false) {
    const assets = [];

    for (const scene of sceneAnalysis.scenes) {
      try {
        console.log(`🔍 Searching media for scene: ${scene.id} (fast mode: ${fastMode})`);
        
        if (fastMode) {
          // Fast mode: use local placeholder instead of downloading
          console.log(`⚡ Using fast mode - creating local placeholder for ${scene.id}`);
          const placeholderPath = await this.createLocalPlaceholder(scene, projectDir);
          ...
          assets.push({
            ...
            originalUrl: null,
            metadata: {
              title: `Placeholder for ${scene.searchKeywords}`,
              provider: 'local',
              duration: scene.duration,
              width: 1280,
              height: 720,
              fileSize: fs.statSync(placeholderPath).size
            }
          });
          console.log(`✅ Fast mode placeholder created for ${scene.id}`);
          continue;
        }
        
        // Normal mode: search for stock media using MCP server
  ...
  // Fast mode local placeholder (no external downloads)
  async createLocalPlaceholder(scene, projectDir) {
    try {
      console.log(`⚡ Creating fast local placeholder for ${scene.id}`);
      return this.createSimplePlaceholder(scene, projectDir);
    } catch (error) {
      console.error('Error creating local placeholder:', error);
      throw error;
    }
  }
