  ...
  buildFilterComplex(timeline) {
    ...
    timeline.scenes.forEach((scene, index) => {
      ...
      if (scene.asset.type === 'image' || scene.asset.type === 'placeholder') {
        // For images: loop for the scene duration and scale/crop to 9:16
        filters.push(`[${inputIndex}:v]loop=loop=-1:size=1:start=0[v${index}loop]`);
        filters.push(`[v${index}loop]scale=720:1280:force_original_aspect_ratio=increase,crop=720:1280,setpts=PTS-STARTPTS,setsar=1[v${index}scaled]`);
        filters.push(`[v${index}scaled]trim=duration=${duration}[v${index}trimmed]`);
      } else {
        // For videos: scale and crop to 9:16, then trim
